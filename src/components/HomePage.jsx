...
const HomePage = ({ setCurrentPage, setSelectedMealCategory, setSelectedPost, t }) => {
    ...
    return (
        <div className="pt-24 pb-16 bg-white dark:bg-[#0b0f19] transition-colors duration-300">
            ...
            {/* Featured AI Tools Container */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-20 lg:mb-24 space-y-6">
                
                {/* AI Food Scanner Featured Strip */}
                <div id="ai-food-scanner">
                    <div 
                        onClick={() => {
                ...
                {/* AI Body Metrics Featured Strip */}
                <div id="ai-body-metrics">
                    <div 
                        onClick={() => {
                            window.history.pushState({}, '', '/ai-body-metrics');
                            window.dispatchEvent(new Event('popstate'));
                            window.scrollTo(0, 0);
                        }}
                        className="group cursor-pointer bg-linear-to-br from-indigo-500 to-purple-600 rounded-[2rem] p-1.5 relative overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1"
                    >
                        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1534438327276-14e5300c3a48?auto=format&fit=crop&q=80&w=1200')] bg-cover bg-center mix-blend-overlay opacity-30 group-hover:opacity-40 transition-opacity duration-700"></div>
                        <div className="bg-slate-900/40 backdrop-blur-md rounded-[1.6rem] p-6 sm:p-10 flex flex-col md:flex-row items-center justify-between gap-6 relative z-10 w-full">
                            <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 text-center sm:text-left w-full">
                                <div className="w-16 h-16 bg-white/20 backdrop-blur-lg rounded-2xl flex items-center justify-center border border-white/20 shrink-0 shadow-inner">
                                ...
                                <div className="text-white">
                                    <h3 className="text-2xl font-black mb-2">New: AI Body Metrics Advisor</h3>
                                    <p className="text-indigo-50 font-medium opacity-90 text-sm sm:text-base leading-snug">Extract local body posture with 100% private, on-device algorithms to generate an actionable clinical strategy.</p>
                                </div>
                            </div>
                            <div className="shrink-0 w-full md:w-auto">
                                <span className="w-full md:w-auto bg-white text-indigo-600 font-bold px-8 py-3.5 rounded-xl flex items-center justify-center gap-2 group-hover:scale-105 transition-transform duration-300 shadow-md">
